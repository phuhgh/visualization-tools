...
export function canvasTestChartGo(emscriptenModule: IEmscriptenWrapper<ICartesian2dBindings>): void
{
    ...
    const chart = CanvasChartFactory.createOne({
        chartContainer: document.getElementById("canvas-chart") as HTMLElement,
    ...
    const interleavedPointTester = {
        ...
        hitTestComponent: new SharedInterleavedLine2dHitTestComponent(SharedInterleavedLine2dQuadIndexerFactory.createOneF64(emscriptenModule)),
    };
...
export function glTestChartGo(emscriptenModule: IEmscriptenWrapper<ICartesian2dBindings>): void
{
    ...
    const interleavedPointTester = {
        hitAllowedComponent: new HitAlwaysAllowedComponent(),
        hitTestComponent: new SharedInterleavedLine2dHitTestComponent(SharedInterleavedLine2dQuadIndexerFactory.createOneF32(emscriptenModule)),
    };
    ...
    const lineGc = CompositeGraphicsComponent
        .createOne(new GlCaplessLineGraphicsComponent(new GlCartesian2dCameraBinder(), new GlInterleaved2dPointBinder(interleavedBindingDescriptor, { pointsToBind: 2 })))
    ...
    {
        const someData = SharedInterleavedConnector.createOneF32(emscriptenModule, 200, interleavedConfig);
