...
if [ "$2" = "release" ]; then
  emcmake cmake -G Ninja .. -DCMAKE_BUILD_TYPE=Release -DBUILD_VISUALIZATION_DOCS=1 || exit
else
  emcmake cmake -G Ninja .. -DCMAKE_BUILD_TYPE=Debug -DBUILD_VISUALIZATION_DOCS=1 || exit
fi
